...
Starte über Pi zwei frische Subagenten parallel mit dem `subagent`-Tool. Beide führen denselben Review-Auftrag aus: den Pi-Skill `review`.
...
- Injiziere den Review-Skill an jedem Task über den `skill`-Parameter: setze `skill: "review"`
...
- Wähle vor dem Start genau ein Thinking-Level für die Review-Agenten. Nutze den Modell-Suffix `:<thinking>` mit folgender Auswahl:
...
- Verwende diese Modell-Overrides mit dem gewählten Thinking-Level:
  - GPT-Review: `model: "openai-codex/gpt-5.6-sol:<thinking>"`
  - Opus-Review: `model: "anthropic/claude-opus-4-8:<thinking>"`
- Beispiel: `model: "openai-codex/gpt-5.6-sol:high"`
...
1. GPT-Review (`openai-codex/gpt-5.6-sol:<thinking>`)
2. Opus-Review (`anthropic/claude-opus-4-8:<thinking>`)

Format:

```markdown
## Review 1 — GPT-5.6 Sol (`openai-codex/gpt-5.6-sol:<thinking>`)
